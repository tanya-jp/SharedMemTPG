...
void eval_tsmt_mpi_main(long t, TPG &tpg, ostringstream &os,
                        mpi::communicator &world) {
  string my_string = "MAIN";
  map<long, team *> rootTeams;
  vector<team *> teamsToEval;
  vector<team *> teamsThisEval;
  ...
  os << "t " << t << " phs " << tpg.phase() << " tToEvl " << numTeamsToEval
     << endl;
  // no more teams to evaluate, so let the rest of the procs know they are not
  // needed this round
  ...
  for (int proc = 1; proc < world.size(); proc++) {
    if (!all_strings[proc].empty()) {
      ...
      while (getline(f, resultLine)) {
        ...
        tpg.setOutcome(rootTeams[rslt_id], tmpBehavSeq, r_runTimeStats,
                       tpg.phase(), t);
        active.clear();
...
void eval_tsmt_mpi_sub(TPG &tpg, mpi::communicator &world,
                       vector<vector<vector<double> > > &data,
                       vector<vector<int> > &pis, int horizon) {
  state *worldState = new state(tpg.dim());
  vector<int> *pisEval;
  deque<double> recursizeSeriesQ;
  vector<double> runTimeStats;
  runTimeStats.reserve(tpg.numPointAuxDouble());
  runTimeStats.resize(tpg.numPointAuxDouble());
  ...
  while (checkpointString.compare("done") != 0) {
    ...
    if (tpg.replay() || (checkpointString.compare("x") != 0 &&
      ...
      for (size_t i = 0; i < teams.size(); i++) {
        ...
        for (size_t tI = 0; tI < pisEval->size(); tI++) {
          ...
          while (sample < tpg.dim()) {
            recursizeSeriesQ.push_back(
                data[tpg.activeTask()][(*pisEval)[tI] + sample][0]);
            sample++;
          }
          ...
          int numSamplesEval =
              tpg.phase() == _TEST_PHASE || tpg.phase() == _VALIDATION_PHASE
                  ? 100
                  : 50;
          while (sample < numSamplesPrime + numSamplesEval - horizon) {
            ...
            if (tpg.replay())
              atomicLearner =
                  tpg.getAction(teams[i], worldState, true, visitedTeams,
                                decisionInstructions, step, winningLearners,
                                decisionFeatures, decisionMemories, teamPath);
            else
              atomicLearner =
                  tpg.getAction(teams[i], worldState, true, visitedTeams,
                                decisionInstructions, step);
            ...
            if (tpg.phase() != _VALIDATION_PHASE ||
                (tpg.phase() == _VALIDATION_PHASE &&
                 sample >= (numSamplesPrime * 2) - horizon)) {
              runTimeStats[tpg.hostFitnessMode()] +=
                  pow((data[tpg.activeTask()][(*pisEval)[tI] + sample + horizon]
                           [0] -
                       (atomicLearner->memGet())->getMem()[0]),
                      2);  // SSE
              runTimeStats[tpg.numFitMode()] += visitedTeams.size();
              runTimeStats[tpg.numFitMode() + 1] += decisionInstructions;
              numScoredPrediction++;
              if (tpg.replay()) {
                cout << "TPG::test id " << teams[i]->id() << " sample "
                     << sample << " input "
                     << data[tpg.activeTask()][(*pisEval)[tI] + sample][0];
                cout << " target "
                     << data[tpg.activeTask()]
                            [(*pisEval)[tI] + sample + horizon][0]
                     << " prediction "
                     << (atomicLearner->memGet())->getMem()[0];
                cout << " visitedTeams " << visitedTeams.size();
                cout << " activeNodes";
                for (auto it = visitedTeams.begin(); it != visitedTeams.end();
                     it++)
                  cout << " h" << (*it)->id();
                for (auto it = winningLearners.begin();
                     it != winningLearners.end(); it++)
                  cout << " s" << (*it)->id();
                cout << " decisionInstructions " << decisionInstructions
                     << " decisionFeatures";
                for (auto it = decisionFeatures.begin();
                     it != decisionFeatures.end(); it++)
                  cout << " " << (*it).size();
                cout << " decisionMemories";
                // get all memories used this decision
                set<memory *> mems;
                for (auto it = decisionMemories.begin();
                     it != decisionMemories.end(); it++)
                ...
                for (auto it = mems.begin(); it != mems.end(); it++) {
                  ...
                  for (auto rwit = memReadTimes.begin();
                       rwit != memReadTimes.end(); rwit++)
                    if (((int)(*rwit)) >= 0)  // ignore untouched registers (-1)
                      memActiveReadT.insert((int)(*rwit));
                  for (auto rwit = memWriteTimes.begin();
                       rwit != memWriteTimes.end(); rwit++)
                    if (((int)(*rwit)) >= 0)  // ignore untouched registers (-1)
                ...
                vector<int> v;
                for (auto it = memActiveReadT.begin();
                     it != memActiveReadT.end(); it++)
                  v.push_back(step - (*it));
        ...
        if (isfinite(runTimeStats[tpg.hostFitnessMode()]))
          runTimeStats[tpg.hostFitnessMode()] =
              -1 * (runTimeStats[tpg.hostFitnessMode()] / numScoredPrediction);
        else {
          runTimeStats[tpg.hostFitnessMode()] =
              numeric_limits<double>::lowest();
          cerr << "TPG::eval infinite SSE " << " team " << teams[i]->id()
               << " atomicAction " << atomicAction << endl;
        }
        runTimeStats[tpg.numFitMode()] =
            runTimeStats[tpg.numFitMode()] /
            numScoredPrediction;  // mean visited teams
        runTimeStats[tpg.numFitMode() + 1] =
            runTimeStats[tpg.numFitMode() + 1] /
            numScoredPrediction;  // mean instruction count (RTC)
        // if (tpg.phase() == _TEST_PHASE)
        //    runTimeStats[tpg.hostFitnessMode()] =
        //    abs(runTimeStats[tpg.hostFitnessMode()]);
        ...
        if (tpg.replay()) {
          vector<behaviourType> tmpBehavSeq;
          tpg.setOutcome(teams[i], tmpBehavSeq, runTimeStats, tpg.phase(),
                         tpg.tPickup());
        }
      }
    }
