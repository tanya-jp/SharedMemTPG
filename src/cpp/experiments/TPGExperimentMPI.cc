...
int main(int argc, char **argv) {
    ...
    if (world.rank() == 0) {  // Master Process
        ...
        chrono::duration<double> endGen =
            chrono::system_clock::now() - startGen;
        ...
        if (tpg.GetParam<int>("replay")) {
            ...
            while (tpg.GetState("t_current") <=
                ...
                endSetEliteTeams =
                    chrono::system_clock::now() - startSetEliteTeams;
                ...
                if (tpg.GetParam<int>("track_experiments") &&
                    ...
                    apiClient->LogMetric("evl", std::to_string(endEval.count()),
                                         "", gen);
                    apiClient->LogMetric(
                        "gTms", std::to_string(endGenTeams.count()), "", gen);
                    apiClient->LogMetric(
                        "elTms", std::to_string(endSetEliteTeams.count()), "",
                        gen);
                    apiClient->LogMetric(
                        "sTms", std::to_string(endSelTeams.count()), "", gen);
                    apiClient->LogMetric(
                        "chkp", std::to_string(endChkp.count()), "", gen);
                    apiClient->LogMetric(
                        "rprt", std::to_string(endReport.count()), "", gen);
                    apiClient->LogMetric(
                        "MDS", std::to_string(endMODES.count()), "", gen);
