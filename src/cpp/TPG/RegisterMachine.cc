...
void RegisterMachine::MarkFeatures(instruction *istr, int in) {
    ...
    if (istr->GetInType(in) == memoryEigen::SCALAR_TYPE) {
        ...
        for (size_t f = istr->GetInIdx(in), row = 0;
             row < istr->GetInMem(in)->memory_size_; row++) {
            for (size_t col = 0; col < istr->GetInMem(in)->memory_size_;
                 col++) {
                // features_.insert(f++ % num_input_);  // toroidal
                features_.insert(f++);
            }
        ...
        for (size_t in = 0; in < 2; in++) {
            ...
            if (istr->GetInType(in) != memoryEigen::NA_TYPE) {
                if (istr->IsMemoryRef(in)) {
                    ...
                    // memoryIndices_ and memory_size_ can be dynamic, so do
                    // mods here.
                    istr->SetInIdxE(in, istr->GetInIdx(in) %
                                            istr->GetInMem(in)->memoryIndices_);

                    // Input is a memory ref. Track read time for temporal
                    // memory.
                    ...
                    istr->SetInMem(
                        in, observation_memory_buff_[istr->GetInType(in)]);
                    // memoryIndices_ and memory_size_ can be dynamic, so do
                    // mods here.
                    istr->SetInIdxE(in, istr->GetInIdx(in) %
                                            istr->GetInMem(in)->memoryIndices_);
