...
double WrapContinuousAction(EvalData &eval) {
    return eval.program_out->privateMemory_[MemoryEigen::kScalarType_]
        ->working_memory_[1](0, 0);
}

double WrapContinuousActionSigmoid(EvalData &eval) {
    double p = eval.program_out->privateMemory_[MemoryEigen::kScalarType_]
                   ->working_memory_[1](0, 0);
    return 1 / (1 + exp(-p));
}

vector<double> WrapVectorActionSigmoid(EvalData &eval) {
    auto mat = eval.program_out->privateMemory_[MemoryEigen::kVectorType_]
                   ->working_memory_[1];
