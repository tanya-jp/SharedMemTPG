 ...
 public:
  ...
  Results update(int actionD, double actionC, mt19937 &rng) {
    ...
    while (!terminal() && count < 4) {
      ...
      d1 =
          m1 * pow(lc1, 2) +
          m2 * (pow(l1, 2) + pow(lc2, 2) + 2 * l1 * lc2 * cos(state[_theta2])) +
          I1 + I2;
      d2 = m2 * (pow(lc2, 2) + l1 * lc2 * cos(state[_theta2])) + I2;

      phi_2 = m2 * lc2 * g * cos(state[_theta1] + state[_theta2] - M_PI / 2.0);
      phi_1 =
          -(m2 * l1 * lc2 * pow(state[_theta2Dot], 2) * sin(state[_theta2]) -
      ...
      theta2_ddot =
          (torque + (d2 / d1) * phi_1 -
           m2 * l1 * lc2 * pow(state[_theta1Dot], 2) * sin(state[_theta2]) -
           phi_2) /
