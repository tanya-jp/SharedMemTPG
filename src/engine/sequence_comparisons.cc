...
double EuclideanDistSqrd(std::vector<double>& x, std::vector<double>& y) {
   double dist = 0;
   std::vector<double>::iterator xiter, yiter, enditer;
...
int hammingDist(std::vector<int>& x, std::vector<int>& y) {
   int dist = 0;
   std::vector<int>::iterator xiter, yiter, enditer;
   for (xiter = x.begin(), yiter = y.begin(), enditer = x.end();
        xiter != enditer; xiter++, yiter++)
...
double calculateTheils_Multi(const std::vector<double>& targets,
   ...
   for (size_t i = 0; i < num_elements - 1; i++) {
      ...
      numerator_duration += pow(
          predictions[i * num_variables + 1] - targets[i * num_variables + 1],
          2);
      ...
      numerator_pitch += pow(
          predictions[i * num_variables + 2] - targets[i * num_variables + 2],
          2);
   ...
   denominator_offset =
       (denominator_offset == 0) ? epsilon : denominator_offset;
...
double calculatePearson_Multi(const std::vector<double>& targets,
   ...
   std::vector<std::vector<double>> x(num_variables);
   std::vector<std::vector<double>> y(num_variables);
   for (auto& v : x)
